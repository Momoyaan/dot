...
export const DNDToggle = (props) =>
	Widget.Button({
		...
		child: Widget.Icon({
			icon: notifications
				.bind("dnd")
				.as((x) => (x ? "preferences-system-notifications-symbolic" : "notifications-disabled-symbolic")),
		}),
...
/** @param {number} time */
const time = (time, format = "%I:%M %p") => GLib.DateTime.new_from_unix_local(time).format(format);
...
export const List = (props) =>
	Widget.Scrollable({
		...
		child: Widget.Box({
			vertical: true,
			children: notifications.bind("notifications").as((x) => x.map(Notification)),
		}),
		...props,
	});
